...
_ACTIONS = {
    ...
    "4": ("summary", "per-group monthly tables", _summary_argv),
    "5": ("sample", "proportional per-category sample to .xlsx", _sample_argv),
    "6": ("filter", "keep rows matching keywords", _filter_argv),
    "7": ("links", "extract and count t.me links", _links_argv),
    "8": ("login", "(re)authorise and save the session", _login_argv),
}
